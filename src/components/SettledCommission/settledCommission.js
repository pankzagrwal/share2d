...
import { getSettledTransaction } from "../AllCommission/actions.js";
...
const AllCommmission = () => {
  ...
  const transactions = useSelector(
    (state) => state?.transaction?.merchant ?? []
  );
  const userId = useSelector((state) => state?.user?.id);
  const history = useHistory();
  React.useEffect(() => {
    dispatch(getSettledTransaction(id));
  }, [id, dispatch]);
              ...
              <>
                <Grid item xs={12}>
                  {transactions.map((item, index) => {
                    let content;
                    const { store_name, phone } = item.second_party_detail;
                ...
                >
                  <Typography
                    ...
                    align="center"
                  >
                    <Link color="secondary" onClick={(e) => e.preventDefault()}>
                      Go Back to un-settled Commission{" "}
                    </Link>
                  </Typography>
                </Grid>
              </>
