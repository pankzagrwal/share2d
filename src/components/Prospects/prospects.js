...
const ProspectComing = ({ prospect, updateLead }) => {
  ...
  const {
    id,
    promoter_detail: { store_name, phone: promoter_phone } = {},
    customer: { name, phone } = {},
    item_description,
  } = prospect;
  ...
  return (
    ...
    >
      ...
      <Grid item container xs={10} alignItems="center">
        ...
        <Grid item container direction="row" alignItems="center">
          <Typography variant="body2">{store_name}</Typography>
          <SmartphoneIcon
            fontSize="small"
            onClick={(evt) => {
              evt.preventDefault();
...
const ProspectSent = ({ prospect }) => {
  const {
    customer: { name } = {},
    item_description,
    business_detail: { store_name = "Merchant Name" } = {},
  } = prospect;
  const classes = useStyles();
  const profile = React.useMemo(() => Math.floor(Math.random() * 6) + 1, []);
  return (
    ...
    >
      ...
      <Grid item container xs={10} alignItems="center">
        ...
        <Grid item container className={classes.textMuted} direction="column">
          <span>{item_description}</span>
          <span>Referred To: {store_name}</span>
        </Grid>
      </Grid>
    </Grid>
  );
...
const Prospects = ({ promoters, business, updateLead }) => {
