...
export default function IndustryDialog({ onClose, selectedIndustry, isOpen }) {
      ...
      <DialogContent>
        <Grid container spacing={2} alignItems="center" justify="center">
          <Grid item xs={12}>
            ...
            >
              {[{ id: "", name: "All Industries" }, ...industriesList].map(
                (item, i) => {
                  return (
                    <MenuItem value={item.id} name={item.name} key={i}>
                      {item.name}
                    </MenuItem>
                  );
                }
