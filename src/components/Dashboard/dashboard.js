...
const Dashboard = () => {
  ...
  // Commission Receive
  const commissionReceive = useSelector(
    (state) => state?.prospects?.commission?.receive ?? []
  );

  // Commission Receive
  const commissionGive = useSelector(
    (state) => state?.prospects?.commission?.give ?? []
  );
        ...
        <Grid container spacing={3} justify="space-between" alignItems="center">
          ...
          <Grid item xs={12}>
            <CurrentProspects
              promoters={promoters}
              business={business}
              updateLead={updateLeadHandle}
            />
          </Grid>
          <Grid item xs={12}>
            <RecentCommission
              receive={commissionReceive}
              give={commissionGive}
              isReadonly
            />
          </Grid>
