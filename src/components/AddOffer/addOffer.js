...
export default function AddOffer() {
  ...
  const { id, type, flat_commission, percentage_commission } = offer;

  const [commission, setCommission] = React.useState(
    flat_commission || percentage_commission
  );
  ...
  return (
    <Container maxWidth="xs" justify="center" className={classes.paper}>
        ...
        <Grid container spacing={2} alignItems="center" justify="center">
          ...
          <Grid item xs={12}>
            <TextField
              ...
              onChange={(evt) => {
                setCommission(evt.target.value);
              }}
              value={commission}
            />
          </Grid>
      ...
      <Typography variant="caption" display="block">
        ...
        if you would like to work as Referrer only.
      </Typography>
    </Container>
  );
}
